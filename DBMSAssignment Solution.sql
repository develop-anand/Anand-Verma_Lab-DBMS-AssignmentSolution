...
-- 1)Display the number of the customer group by their genders who have placed any order of amount greater than or equal to Rs.3000.
...
-- 2) Display all the orders along with the product name ordered by a customer having Customer_Id=2.
...
-- 4) Find the category of the product whose order amount is minimum.
...
FROM
    PRODUCT
        JOIN
    product_details ON PRODUCT.PRO_ID = product_details.PRO_ID
        JOIN
    `ORDER` ON product_details.PROD_ID = `ORDER`.PROD_ID
WHERE
...
/* 7.) Create a stored procedure to display the Rating for a Supplier if any along with the Verdict 
on that rating if any like if rating >4 then “Genuine Supplier” 
